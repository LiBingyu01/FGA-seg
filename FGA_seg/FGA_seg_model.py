...
def visualize_and_save(tensor, name, save_dir=""):
    ...
    # if tensor.dim() == 5:
    #     tensor = tensor[0, 0]  # Take the first batch and channel
    # elif tensor.dim() == 4:
    #     tensor = tensor[0]  # Take the first batch
    B = tensor.shape[0]
    for i in range(B):
        image_numpy = tensor[i].permute(1, 2, 0).cpu().numpy()
        image_numpy = image_numpy.clip(0, 1)
        # 可视化图像
        plt.imshow(image_numpy)
        plt.axis('off')  # 关闭坐标轴
        plt.title('RGB Image')
        save_path = f"{save_dir}/{name}_visualization_batch_{i}.png" if save_dir else f"{name}_visualization.png"
        plt.imsave(save_path, image_numpy)
        print(f"Image saved to {save_path}")


@META_ARCH_REGISTRY.register()
class FGASeg(nn.Module):
    @configurable
    def __init__(
        self,
        *,
        ...
        self.upsample1 = nn.ConvTranspose2d(self.proj_dim, 256, kernel_size=2, stride=2)    # H*2 W*2
        self.upsample2 = nn.ConvTranspose2d(self.proj_dim, 128, kernel_size=4, stride=4)    # H*4 W*4
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "decoder_mod": cfg.MODEL.SEM_SEG_HEAD.DECODER_MOD,
            "lambda_auxi": cfg.MODEL.SEM_SEG_HEAD.LAMBDA_AUXI,
            "lambda_align": cfg.MODEL.SEM_SEG_HEAD.LAMBDA_ALIGN
        }
    ...
    def forward(self, batched_inputs):
        ...
        print("clip_images_resized_clip_images_resized.shape",clip_images_resized.shape)
        visualize_and_save(clip_images_resized,"clip_images_resized", "/data01/lby/FGA-Seg/visulization/rgb")
        ...
        res4 = self.upsample1(res4) # 48*48
        res5 = self.upsample2(res5) # 96*96
        features = {'res5': res5, 'res4': res4, 'res3': res3,}
        

        if self.training:
            outputs, mask_aux0, mask_aux1, mask_aux2, out_text = self.sem_seg_head(clip_features, features)
        else:
        ...
        if self.training:
            ...
            if isinstance(out_text,int):
                loss_align = 0
                # print("----")
            else:
                if out_text.shape[-2:] != _targets.permute(0,3,1,2).shape[-2:]:
                    _targets_ = F.interpolate(_targets.permute(0,3,1,2), out_text.shape[-2:], mode='nearest').permute(0,2,3,1).detach()
                loss_align = F.binary_cross_entropy_with_logits(out_text.permute(0,2,3,1), _targets_)
            # ---------------- auxi loss
            if self.decoder_mod == "fast":
                mask_aux0 = F.interpolate(mask_aux0, size=(targets.shape[-2], targets.shape[-1]), mode="bilinear", align_corners=False)
                mask_aux1 = F.interpolate(mask_aux1, size=(targets.shape[-2], targets.shape[-1]), mode="bilinear", align_corners=False)
                mask_aux2 = F.interpolate(mask_aux2, size=(targets.shape[-2], targets.shape[-1]), mode="bilinear", align_corners=False)
                mask_aux0 = mask_aux0.permute(0,2,3,1)
                loss_0 = F.binary_cross_entropy_with_logits(mask_aux0, _targets)
                mask_aux1 = mask_aux1.permute(0,2,3,1)
                loss_1 = F.binary_cross_entropy_with_logits(mask_aux1, _targets)
                mask_aux2 = mask_aux2.permute(0,2,3,1)
                loss_2 = F.binary_cross_entropy_with_logits(mask_aux2, _targets)
                loss_all = loss + self.lambda_auxi*loss_0 + loss_1 + loss_2 + self.lambda_align*loss_align
            elif self.decoder_mod == "no_fast":
                loss_all = loss + self.lambda_align*loss_align

            losses = {"loss_sem_seg" : loss_all}
            return losses
