...
class DecoderResNetLayer(nn.Module):
...
def conv_layer_cat(in_dim, out_dim, kernel_size=1, padding=0, stride=1):
...
def visualize_and_save(tensor, name, save_dir=""):
    # Handle 5D tensors by taking a slice
    tensor = tensor.squeeze(1)  # Take the first batch and channel
    B = tensor.shape[0]
...
class Decoder(nn.Module):
    def __init__(self, 
        ...
        Args:
            ...
            num_layers: Number of layers for the Decoder
    ...
    def conv_decoder(self, corr_embed, corr_local_embed, guidance):
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        print("corr_global_corr_global.shape",corr_global.shape)
        corr_local = self.Local_align(img_feats, text_feats).unsqueeze(1) # bpthw
        print("corr_local_corr_local.shape",corr_local.shape)

        visualize_and_save(corr_global,"corr_global","/data01/lby/FGA-Seg/visulization/global")
        visualize_and_save(corr_local,"corr_local","/data01/lby/FGA-Seg/visulization/local")
        ...
        # print("---- corr_embed --------",corr_embed.shape)
        ...
        logit = self.conv_decoder(corr_embed, corr_local_embed, projected_decoder_guidance)
