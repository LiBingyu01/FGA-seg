...
class Decoder_fastLayer(nn.Module):
...
class Decoder_fast(nn.Module):
    def __init__(self, 
        ...
        """
        Cost Aggregation Model for FGA-Seg
        Args:
            ...
            num_layers: Number of layers for the Decoder_fast
        ...
        if self.fusion_mod == "cat":
            self.conv_cat = nn.Linear(hidden_dim*2, hidden_dim)
            self.conv_cat_decoder_1 = nn.Linear(hidden_dim*2, hidden_dim)
            self.conv_cat_decoder_2 = nn.Linear(hidden_dim, int(hidden_dim/2))

        self.corr_local_decoder_projection = nn.Sequential(
            nn.ConvTranspose2d(hidden_dim, int(hidden_dim/2), kernel_size=2, stride=2),
            DoubleConv(int(hidden_dim/2), int(hidden_dim/2)),
            nn.ReLU(),
        )
        ...
        self.head_auxi_0 = nn.Conv2d(hidden_dim, 1, kernel_size=3, stride=1, padding=1)
        self.head_auxi_1 = nn.Conv2d(hidden_dim, 1, kernel_size=3, stride=1, padding=1)
        self.head_auxi_2 = nn.Conv2d(int(hidden_dim/2), 1, kernel_size=3, stride=1, padding=1)
        
        self.pad_len = pad_len
        
        if clip_pretrained == "ViT-B/16":
            self.Local_align = Local_align(word_dim=512, vision_dim=512, kernel_size=kernel_size)
        else:
            self.Local_align = Local_align(word_dim=768, vision_dim=768, kernel_size=kernel_size)
    ...
    def corr_embed_local(self, x):
        B = x.shape[0]
        corr_embed = rearrange(x, 'B P T H W -> (B T) P H W')
        corr_embed = self.conv2(corr_embed)
        corr_embed = rearrange(corr_embed, '(B T) C H W -> B C T H W', B=B)
        return corr_embed

    def conv_decoder_training(self, corr_embed, corr_local_embed, projected_guidance, projected_text_guidance, guidance):
        B = corr_embed.shape[0]
        T = corr_embed.shape[2]
        # ====================== aggragation 
        if self.fusion_mod == "cat":
            corr_embed = self.conv_cat(torch.cat([corr_embed, corr_local_embed], dim=1).permute(0,2,3,4,1)).permute(0,4,1,2,3)
        elif self.fusion_mod == "add":
            corr_embed = corr_embed + corr_local_embed
        corr_embed0 = rearrange(corr_embed, 'B C T H W -> (B T) C H W')
        mask_aux0 = self.head_auxi_0(corr_embed0)
        mask_aux0 = rearrange(mask_aux0, '(B T) () H W -> B T H W', B=B)
        for layer in self.layers: # 1*768*24*24
            corr_embed = layer(corr_embed, projected_guidance, projected_text_guidance)

        # ====================== decoder 1
        corr_embed = rearrange(corr_embed, 'B C T H W -> (B T) C H W')
        corr_local_embed = rearrange(corr_local_embed, 'B C T H W -> (B T) C H W')
        # decoder 1
        if self.fusion_mod == "cat":
            corr_embed = self.conv_cat_decoder_1(torch.cat([corr_embed, corr_local_embed], dim=1).permute(0,2,3,1)).permute(0,3,1,2)
        elif self.fusion_mod == "add":
            corr_embed = corr_embed + corr_local_embed
        mask_aux1 = self.head_auxi_1(corr_embed)
        mask_aux1 = rearrange(mask_aux1, '(B T) () H W -> B T H W', B=B)
        corr_embed = self.decoder1(corr_embed, guidance[0])

        # ====================== decoder 2
        corr_local_embed = self.corr_local_decoder_projection(corr_local_embed)
        if self.fusion_mod == "cat":
            corr_embed = self.conv_cat_decoder_2(torch.cat([corr_embed, corr_local_embed], dim=1).permute(0,2,3,1)).permute(0,3,1,2)
        elif self.fusion_mod == "add":
            corr_embed = corr_embed + corr_local_embed
        mask_aux2 = self.head_auxi_2(corr_embed)
        mask_aux2 = rearrange(mask_aux2, '(B T) () H W -> B T H W', B=B)
        corr_embed = self.decoder2(corr_embed, guidance[1])

        # ====================== head
        corr_embed = self.head(corr_embed)
        output = rearrange(corr_embed, '(B T) () H W -> B T H W', B=B)

        return output, mask_aux0, mask_aux1, mask_aux2

    def conv_decoder_inference(self, corr_embed, corr_local_embed, projected_guidance, projected_text_guidance, guidance, topk=8):
        ...
        mask_aux0 = self.head_auxi_0(corr_embed0)
        mask_aux0 = rearrange(mask_aux0, '(B T) () H W -> B T H W', B=B)
        aux_valid0 = self.get_valid_idx_from_mask(mask_aux0, topK=topk)
        # aux_valid0 = aux_valid0.reshape(-1)
        # corr_embed0 = corr_embed0[aux_valid0]
        # T = corr_embed0.shape[0]
        # corr_embed = rearrange(corr_embed0, '(B T) C H W -> B C T H W', T=T)
        corr_embed = torch.stack([corr_embed[i, :, aux_valid0[i], :, :] for i in range(B)], dim=0)
        projected_text_guidance = torch.stack([projected_text_guidance[i, aux_valid0[i], :] for i in range(B)], dim=0)
        # projected_text_guidance.view(-1, self.hidden_dim)[aux_valid0.reshape(-1),:].view(B,-1,self.hidden_dim)
        for layer in self.layers: # 1*768*24*24
            corr_embed = layer(corr_embed, projected_guidance, projected_text_guidance)

        corr_embed = rearrange(corr_embed, 'B C T H W -> (B T) C H W')
        corr_local_embed = rearrange(corr_local_embed, 'B C T H W -> (B T) C H W')
        corr_local_embed = corr_local_embed[aux_valid0.reshape(-1)]
        corr_embed = self.conv_cat_decoder_1(torch.cat([corr_embed, corr_local_embed], dim=1).permute(0,2,3,1)).permute(0,3,1,2)
        mask_aux1 = self.head_auxi_1(corr_embed)
        mask_aux1 = rearrange(mask_aux1, '(B T) () H W -> B T H W', B=B)
        aux_valid1 = self.get_valid_idx_from_mask(mask_aux1, topK=topk)
        aux_valid1 = aux_valid1.reshape(-1)
        corr_embed = self.decoder1(corr_embed[aux_valid1], guidance[0])


        # decoder 2
        corr_local_embed = self.corr_local_decoder_projection(corr_local_embed[aux_valid1])
        corr_embed = self.conv_cat_decoder_2(torch.cat([corr_embed, corr_local_embed], dim=1).permute(0,2,3,1)).permute(0,3,1,2)
        mask_aux2 = self.head_auxi_2(corr_embed)
        mask_aux2 = rearrange(mask_aux2, '(B T) () H W -> B T H W', B=B)
        aux_valid2 = self.get_valid_idx_from_mask(mask_aux2, topK=topk)
        aux_valid2 = aux_valid2.reshape(-1)
        corr_embed = self.decoder2(corr_embed[aux_valid2], guidance[1])
        ...
        # 复制有效索引aux_valid
        valid_idx1 = aux_valid0.clone()
        valid_idx2 = aux_valid1.clone()
        valid_idx2[aux_valid1] = aux_valid2
        valid_idx1[aux_valid0] = valid_idx2

        _, _, H, W = corr_embed.size()
        output = torch.zeros([B, T, H, W]).cuda()
        valid_idx1 = valid_idx1.reshape(B, T)
        for i in range(B):
            output[i, valid_idx1[i]] = corr_embed[i].sigmoid() 
    ...
    def get_valid_idx_from_mask(self, mask, topK=2):
        ...
        valid_idx = valid_idx.repeat(B,1)
        return valid_idx
    ...
    def forward(self, img_feats, text_feats, appearance_guidance):
        ...
        corr_global = self.correlation(img_feats, text_feats) # bpthw
        corr_local = self.Local_align(img_feats, text_feats).unsqueeze(1) # bpthw

        # 计算相似性矩阵
        classes = None
        if self.pad_len > 0 and text_feats.size(1) > self.pad_len:
            avg = corr_global.permute(0, 2, 1, 3, 4).flatten(-3).max(dim=-1)[0] 
            ...
            img_feats = F.normalize(img_feats, dim=1) # B C H W
            text_feats = th_text
            corr_global = torch.einsum('bchw, btpc -> bpthw', img_feats, text_feats)
            corr_local = self.Local_align(img_feats, text_feats).unsqueeze(1) # bpthw

        # embedding
        corr_embed = self.corr_embed(corr_global)
        corr_local_embed = self.corr_embed_local(corr_local)
        ...
        if self.guidance_projection is not None: # 
            projected_guidance = self.guidance_projection(appearance_guidance[0]) # 1*768*24*24
        if self.decoder_guidance_projection is not None: # decoder 1*768*48*48  1*768*96*96
        ...
        if self.text_guidance_projection is not None:
            ...
            projected_text_guidance = self.text_guidance_projection(text_feats) # 768 -> 128

        # upsample
        # 1*768*24*24 ---> 1*768*48*48  1*768*96*96
        if self.training:
            logit, mask_aux0, mask_aux1, mask_aux2 = self.conv_decoder_training(corr_embed, corr_local_embed, projected_guidance, projected_text_guidance, projected_decoder_guidance)
            if classes is not None:
                ...
                logit = out
            return logit, mask_aux0, mask_aux1, mask_aux2
        else:
            select_topk = 16
            logit = self.conv_decoder_inference(corr_embed, corr_local_embed, projected_guidance, projected_text_guidance, projected_decoder_guidance, select_topk)
            if classes is not None:
                out = torch.full((logit.size(0), orig_clases, logit.size(2), logit.size(3)), -100., device=logit.device)
                out.scatter_(dim=1, index=classes[..., None, None].expand(-1, -1, logit.size(-2), logit.size(-1)), src=logit)
                logit = out
